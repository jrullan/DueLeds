/*-------------------------------------------------------------------------
  ...
  the Adafruit_GFX graphics library.  Requires both the Adafruit_NeoPixel2
...
Adafruit_NeoMatrix2::Adafruit_NeoMatrix2(int w, int h, uint8_t *leds_data, uint8_t pin,
  uint8_t matrixType, uint8_t ledType) : Adafruit_GFX(w, h),
  Adafruit_NeoPixel2(w * h, leds_data, pin, ledType), type(matrixType), matrixWidth(w),
  matrixHeight(h), tilesX(0), tilesY(0), remapFn(NULL) { }

// Constructor for tiled matrices:
Adafruit_NeoMatrix2::Adafruit_NeoMatrix2(uint8_t mW, uint8_t mH, uint8_t tX, uint8_t *leds_data,
  uint8_t tY, uint8_t pin, uint8_t matrixType, uint8_t ledType) :
  Adafruit_GFX(mW * tX, mH * tY), Adafruit_NeoPixel2(mW * mH * tX * tY, leds_data, pin,
...
uint16_t Adafruit_NeoMatrix2::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint16_t)(r & 0xF8) << 8) |
         ((uint16_t)(g & 0xFC) << 3) |
                    (b         >> 3);
}

void Adafruit_NeoMatrix2::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_NeoMatrix2::fillScreen(uint16_t color) {
  uint16_t i, n;
  uint32_t c24;
...
void Adafruit_NeoMatrix2::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}
