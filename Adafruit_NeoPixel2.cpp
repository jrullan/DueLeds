...
#include "Adafruit_NeoPixel2.h"
									                            //NOTE: Totem struct is 4 Bytes per Pixel
Adafruit_NeoPixel2::Adafruit_NeoPixel2(uint16_t n, uint8_t *leds, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 4), pin(p), pixels(leds), type(t)
#ifdef __AVR__
  ,port(portOutputRegister(digitalPinToPort(p))),
   pinMask(digitalPinToBitMask(p))
#endif
{
	//fourth (n=3) strip chosen as matrix.print target HARDCODE Dev edition.
	//pixels = leds + 0*numLEDs;
...
Adafruit_NeoPixel2::~Adafruit_NeoPixel2() {
  if(pixels) free(pixels);
  pinMode(pin, INPUT);
}

void Adafruit_NeoPixel2::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void Adafruit_NeoPixel2::show(void) {
...
void Adafruit_NeoPixel2::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void Adafruit_NeoPixel2::setPixelColor(
 uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
  if(n < numLEDs) {
    ...
    uint8_t *p = &pixels[n * 4];    //NOTE: TOTEM Transposition manages data vector in 4 Byte per Pixel logic #1.
    p[rOffset] = r;
    p[gOffset] = g;
    p[bOffset] = b;
...
void Adafruit_NeoPixel2::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
    ...
    //myDueLeds->setLed(n,{r,g,b});
    
    uint8_t *p = &pixels[n * 4];	//NOTE: TOTEM Transposition manages data vector in 4 Byte per Pixel logic #2.
    p[rOffset] = r;
    p[gOffset] = g;
    p[bOffset] = b;
  }
...
uint32_t Adafruit_NeoPixel2::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Adafruit_NeoPixel2::getPixelColor(uint16_t n) const {

  if(n < numLEDs) {
    uint8_t *p = &pixels[n * 4];	//NOTE: TOTEM Transposition manages data vector in 4 Byte per Pixel logic #3.
...
uint8_t *Adafruit_NeoPixel2::getPixels(void) const {
  return pixels;
}

uint16_t Adafruit_NeoPixel2::numPixels(void) const {
  return numLEDs;
}
...
void Adafruit_NeoPixel2::setBrightness(uint8_t b) {
